...
# set up buzzer
buzzer = pwmio.PWMout(board.MISO, frequency=1000, duty_cycle=2^^15)
...
haptic = adafruit_drv2605.DRV2605(i2c) # address 0x5A / 90
haptic.use_LRM()
haptic.sequence[0] = adafruit_drv2605.Effect(1) # effect 1: strong click, 27: short double click strong, 16: 1000 ms alert
#haptic.play()

accelerometer = adafruit_lis3dh.LIS3DH_I2C(i2c, address=25) # address 25 / 0x19

light_sensor = LTR329(i2c) # address 0x29 / 41
light = light_sensor.visible_plus_ir_light - light_sensor.ir_light

battery_monitor = MAX17048(board.I2C()) # address 0x36 / 54
print(f"Battery voltage: {battery_monitor.cell_voltage:.2f} Volts")
print(f"Battery percentage: {battery_monitor.cell_percent:.1f} %")
