...
class MPR121:
    ...
    def reset(self) -> None:
        ...
        for i in range(12):
            self._write_register_byte(MPR121_TOUCHTH_0 + 2 * i, 3)
            self._write_register_byte(MPR121_RELEASETH_0 + 2 * i, 1)

        # Configure baseline filtering control registers.
        self._write_register_byte(MPR121_MHDR, 0x01)
        self._write_register_byte(MPR121_NHDR, 0x01)
        self._write_register_byte(MPR121_NCLR, 0x04)
        self._write_register_byte(MPR121_FDLR, 0x10)
        self._write_register_byte(MPR121_MHDF, 0x01)
        self._write_register_byte(MPR121_NHDF, 0x01)
        self._write_register_byte(MPR121_NCLF, 0x10)
        self._write_register_byte(MPR121_FDLF, 0x80)
        self._write_register_byte(MPR121_NHDT, 0x01)
        self._write_register_byte(MPR121_NCLT, 0x10)
        self._write_register_byte(MPR121_FDLT, 0x80)

        # Set other configuration registers.
        self._write_register_byte(MPR121_DEBOUNCE, 0)
        self._write_register_byte(MPR121_CONFIG1, 0x20)  # 32 uA charge current (default 16uA charge current)
        self._write_register_byte(MPR121_CONFIG2, 0x20)  # 0.5uS encoding, 1ms period

        # set up autoconfig
        self._write_register_byte(MPR121_AUTOCONFIG0, 0x1F) # autoconfig on, auto-reconfig on, preload full baseline register, 2 retries, default filtering
        self._write_register_byte(MPR121_AUTOCONFIG1, 0x00) # do not skip charge time search, interrupts off
        self._write_register_byte(MPR121_UPLIMIT, 202) # limit values for 3.3 V
        self._write_register_byte(MPR121_LOWLIMIT, 131)
        self._write_register_byte(MPR121_TARGETLIMIT, 182)

        # Enable all electrodes.
        self._write_register_byte(MPR121_ECR, 0xCF)  # enable all electrodes, start baseline tracking (initialize with 10 bits: CF, 5 bits: 8F, 0 bits: 0F)
